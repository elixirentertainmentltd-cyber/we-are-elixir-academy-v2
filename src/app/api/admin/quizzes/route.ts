...
}).refine((value) => value.options.filter((option) => option.correct).length === 1, 'Each question must have exactly one correct option.');
const schema = z.object({
  lessonId: z.string().min(1),
  title: z.string().min(2),
  instructions: z.string().optional().default(''),
  passScore: z.number().int().min(1).max(100),
  attemptLimit: z.number().int().min(1).nullable(),
  published: z.boolean(),
...
export async function POST(request: Request) {
  ...
  const quiz = await db.$transaction(async (tx) => {
    const saved = await tx.quiz.upsert({
      ...
      update: { title: data.title, instructions: data.instructions, passScore: data.passScore, attemptLimit: data.attemptLimit, published: data.published },
      create: { lessonId: data.lessonId, title: data.title, instructions: data.instructions, passScore: data.passScore, attemptLimit: data.attemptLimit, published: data.published },
    });
    await tx.quizQuestion.deleteMany({ where: { quizId: saved.id } });
    for (let questionIndex = 0; questionIndex < data.questions.length; questionIndex += 1) {
    ...
    return tx.quiz.findUniqueOrThrow({ where: { id: saved.id }, include: { questions: { include: { options: { orderBy: { position: 'asc' } } }, orderBy: { position: 'asc' } } } });
  });
