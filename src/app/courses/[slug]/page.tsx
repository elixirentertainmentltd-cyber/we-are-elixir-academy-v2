...
import { BookOpen, CheckCircle2, Clock3, HelpCircle, PlayCircle } from 'lucide-react';
...
export default async function CoursePage({params}:{params:Promise<{slug:string}>}){
 ...
 const course=await db.course.findFirst({where:{slug,status:'PUBLISHED'},include:{category:true,modules:{orderBy:{position:'asc'},include:{lessons:{orderBy:{position:'asc'},include:{quiz:{select:{id:true,published:true,title:true}}}}}}}}); if(!course)notFound();
 ...
 <div className="module-list">{course.modules.map((module,index)=><section className="module-card" key={module.id}><div className="module-heading"><span>{String(index+1).padStart(2,'0')}</span><div><h2>{module.title}</h2>{module.description&&<p>{module.description}</p>}</div></div><div className="lesson-list">{module.lessons.map((lesson,lessonIndex)=><Link className="lesson-row" key={lesson.id} href={`/courses/${course.slug}/${lesson.slug}`}><span className={completed.has(lesson.id)?'lesson-check done':'lesson-check'}>{completed.has(lesson.id)?'✓':lessonIndex+1}</span><div><strong>{lesson.title}</strong><small>{lesson.summary}</small>{lesson.quiz?.published && <span className="lesson-quiz-badge"><HelpCircle /> Quiz included</span>}</div><span>{lesson.estimatedMinutes} min</span></Link>)}</div></section>)}</div></Shell>
}
