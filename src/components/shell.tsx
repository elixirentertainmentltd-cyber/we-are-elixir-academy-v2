...
import { Award, BarChart3, BookOpen, GraduationCap, LayoutDashboard, Menu, PencilRuler, ShieldCheck, Users } from 'lucide-react';
...
export function Shell({ children, user }: { children: React.ReactNode; user: { name: string; role: string } }) {
  const links = <>
    ...
    <Link href="/certificates"><Award /> Certificates</Link>
  ...
  return <div className="app-shell"><header><Link href="/dashboard" className="brand"><span className="brand-mark"><GraduationCap /></span><span>Elixir Academy</span></Link><details className="mobile-menu"><summary aria-label="Open navigation"><Menu /></summary><nav>{links}<LogoutButton /></nav></details><nav className="desktop-nav">{links}<span className="user-chip">{user.name}</span><LogoutButton /></nav></header><main className="container">{children}</main><footer><span>We Are Elixir Academy</span><span>Learn. Grow. Create.</span></footer></div>;
}
